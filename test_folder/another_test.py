...
# ####################################
# Basically see the file utils.py for lots of help on the definitions
# Find it in Karla Aceves>>OpenAeroStruct>>openaerostruct>>geometry>>utils.py
...
indep_var_comp.add_output('v', val=23., units='m/s') # Freestream Velocity
...
indep_var_comp.add_output('Mach_number', val=0.067) # Freestream Mach number
indep_var_comp.add_output('re', val=1.403e6, units='1/m') # Freestream Reynolds number
...
surface = {
            ...
            'CD0' : 0.0172,            # CD of the surface at alpha=0

            # Airfoil properties for viscous drag calculation
            'k_lam' : 0.625,         # percentage of chord with laminar
                                    # flow, used for viscous drag
            't_over_c' : 0.12,      # thickness over chord ratio (NACA0015)
            'c_max_t' : .3003,       # chordwise location of maximum (NACA0015)
                                    # thickness
